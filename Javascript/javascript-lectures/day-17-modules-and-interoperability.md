...
1. **Definition:** Explain live bindings with a counter module.
...
2. **Trace:** Two modules import each other and read exported `const` values during initialization. Explain the likely failure.
...
3. **Implementation:** Design module boundaries for a service with validation, database access, and formatting.
...
4. **Debugging:** An ESM module works in development but fails when loaded through CommonJS. Diagnose without assuming a universal interop rule.
...
5. **Design:** Review a module graph with several cycles and top-level network calls.
